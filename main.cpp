...
int main(int argc, char* argv[])
{
    ...
    char c;
    while((c = getopt(argc, argv,"diDIb:o:")) !=EOF )
    {
        switch (c)
        {
    ...
    while(fscanf(fpbam, "%s%d%lf%lf%s", bamFilename, &readLen, &insertSize, &insertSizeStd, svFilename) != EOF)
    {
        ...
        if(mode == 0)
            feature.deletionFeature();
        if(mode == 1)
            feature.insertionFeature();
        if(mode == 2)
            feature.indelFeature();
        fprintf(stderr, "[SVfeature_main] The %d: %s finished.\n\n", indiID++, bamFilename);
        delete bam;
        delete indelList;
        fclose(fpout);
        fclose(fpstats);
